...
const Stories = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
          try {
            const data = await client.fetch('*[_type == "stories" && _updatedAt < now()]|order(_updatedAt desc){title, slug, featuredImage, author, aiAssistant, "category": category[]->{_id, name}}');
            setStories(data);
            setIsLoading(false);
          } catch (error) {

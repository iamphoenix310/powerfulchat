...
const WomenWallpapers: React.FC<WallpaperDetailProps> = ({ womenWalls }) => {
  const {womenWalls: womenSlug} = {womenWalls};
    ...
    const fetchWallpaperData = async () => {
      try {
        const response = await client.fetch(
          `*[_type == "wallpaper" && slug.current == $womenSlug][0]{
            title,
            description,
            tags,
            body,
  ...
  return (
    ...
    {showLoginModal && <LoginModal onClose={() => setShowLoginModal(false)} />}
      ...
      <div className='flex flex-col justify-center items-center mt-5 mb-2'>
        ...
        {wallpaperDetail ? (
          <>
            ...
            <Link href={`/wallpapers/women-wallpapers`}>Back</Link>
      ...
      <SimilarWallpapers category='f03f3672-e7f8-452c-9c97-06dbadf4a4a9'
      categorySlug='women-wallpapers'
      />
          </>
        ) : (
...
export default WomenWallpapers;
